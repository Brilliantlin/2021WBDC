...
###将句子根据delimiter划分为多个句子，每个句子保留标点 zen来源
...
import copy
#取差集(从data1中过滤data1在data2中存在的行)data1-data2：
def getDataDiff(data1,data2):
    df1 = copy.deepcopy(data1)
    df2 = copy.deepcopy(data2)
    df1 = df1.append(df2)
    # 注意两次添加，否则出现问题
    df1 = df1.append(df2)
    df1 = df1.drop_duplicates(subset=df1.columns.tolist(),keep=False)
    return df1
#取交集：print())
def getDataIntersection(data1,data2):
    df1 = copy.deepcopy(data1)
    df2 = copy.deepcopy(data2)
    res = pd.merge(df1,df2,on=df1.columns.tolist())
    return res
#取并集：
def getDataUnion(data1,data2):
    df1 = copy.deepcopy(data1)
    df2 = copy.deepcopy(data2)
